...
public class OccultistTableBlock extends WorkstationBlock {
        ...
        if (world.getBlockEntity(pos) instanceof OccultistTableBlockEntity tile && !player.isCreative()) {
            ...
            if (world.isClient) {
                if (player.isSneaking()) {
                    ...
                    createParticleSpiral(world, pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D, 0.0D, 0.0D, 0.0D, 250, ModParticles.EXPERIENCE_PARTICLE, world.random);
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        if (state.get(FILLING) > 0 && random.nextInt(3) == 0) {
            world.addParticle(ModParticles.EXPERIENCE_PARTICLE, pos.getX() + 0.5D + random.nextDouble() - random.nextDouble(), pos.getY() + 1.0D + random.nextDouble(), pos.getZ() + 0.5D + random.nextDouble() - random.nextDouble(), 0.0D, 0.05D, 0.0D);
        }
    }
