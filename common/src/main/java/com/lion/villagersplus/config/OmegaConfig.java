...
import com.lion.villagersplus.platform.ConfigDirectory;
...
public class OmegaConfig {
        ...
        try {
            ...
            if (!configExists(config)) {
                ...
                try {
                    ...
                    VillagersPlus.LOGGER.error(e.toString());
                    VillagersPlus.LOGGER.info(String.format("Encountered an error while reading %s config, falling back to default values.", config.getName()));
                    VillagersPlus.LOGGER.info(String.format("If this problem persists, delete the config file %s and try again.", config.getName() + "." + config.getExtension()));
                }
            }
    ...
    public static <T extends Config> void writeConfig(Class<T> configClass, T instance) {
        ...
        try {
            ...
            VillagersPlus.LOGGER.error(ioException.toString());
            VillagersPlus.LOGGER.info(String.format("Write error, using default values for config %s.", configClass));
        }
    }
